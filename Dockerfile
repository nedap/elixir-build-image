...
RUN if [ ! "${buildpack_tag}" = "stretch" ]; then bash -c "sed -i 's/http:/https:/g' /etc/apt/sources.list; sed -i 's/deb.debian.org/archive.debian.org/g' /etc/apt/sources.list"; sed -i 's/security.debian.org/archive.debian.org/g' /etc/apt/sources.list" fi;
